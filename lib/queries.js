...
export async function buscarMovimiento({ query = '' } = {}) {
  ...
  // Cuando el resultado es un solo movimiento, el ticket necesita saber si
  // vale la pena ofrecer "ver más cargos de este comercio" — si es el único
  // que existe en todo el historial, ese botón solo repetiría el mismo
  // ticket una y otra vez.
  const totalMismoComercio =
    resultados.length === 1
      ? movimientos.filter(
          (movimiento) => movimiento.comercio.toLowerCase() === resultados[0].comercio.toLowerCase()
        ).length
      : null

  return { cuenta, query, movimientos: resultados, totalMismoComercio }
}
